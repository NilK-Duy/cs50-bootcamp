...
int main(int argc, string argv[])
{
    ...
    // Set the number of preferences[i][j] pairs to 0
    for (int i = 0; i < candidate_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
...
// Check for cycle in locked pairs
bool has_cycle(int winner, bool visited[])
{
    if (visited[winner])
    {
        return true;
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (locked[winner][i] && has_cycle(i, visited))
        {
            return true;
        }
    }
